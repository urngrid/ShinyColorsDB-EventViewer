    ...
    _changeBgByEffect(bgName, effectName, bgEffectTime) {
        ...
        switch (effectName) {
            case "fade":
                ...
                //修改标记 检查背景名 用来判断是否为纯黑
                let origBgName;

                for (const [bgName, sprite] of this._bgMap.entries()) {
                    if (sprite === origBg) {
                        console.log("origBg corresponds to bgName:", bgName);
                        origBgName = bgName;
                    }
                }
                //修改标记完
                ...
                if (this._container.children.length != 1) {
                    ...
                    Utilities.fadingEffect(origBg, {
                        alpha: 0,
                        //修改标记当从00000纯黑背景fade进入新背景时黑背景淡出会因为开启了抗锯齿留下黑对角线，姑且作为特例将时长缩短来消除黑线
                        // time:bgEffectTime ? bgEffectTime : 1000,
                        // time: origBgName == "00000" ? 1 : bgEffectTime ? bgEffectTime : 1000, //当前状态 背景淡变切换时有抗锯齿黑对角线
                        time: bgEffectTime ? bgEffectTime : 1000, //测试
                        //
                        ease: "none",
                        type: "to",
                    });
            ...
            case "blur":
                ...
                const interval = setInterval(() => {
                    ...
                    if (progress <= halfDuration) {
                        ...
                        blurFilter.blur = 24 - 24 * ((progress - halfDuration) / halfDuration);
                    } else {
