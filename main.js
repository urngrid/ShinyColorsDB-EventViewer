...
class AdvPlayer {
  ...
  async loadTrackScript(Track) {
    ...
    if (typeof Track === "string") {
      return new Promise((res, rej) => {
        this._app.loader
          .add("eventJson", `${assetUrl}/json/${Track}`)
          .load((_, resources) => {
  ...
  _searchFromMasterList(jsonPath) {
    return new Promise((res, rej) => {
      this._app.loader
        .add("TranslateMasterList", translate_master_list)
        .load((_, resources) => {
          let translateUrl = this._getCSVUrl(
            resources.TranslateMasterList.data,
            jsonPath
          );
          res(translateUrl);
        });
  ...
  _getCSVLocal = async (jsonPath) => {
    // 替换 .json 为 .csv
    const filePath = `${local_translate_CSV_path}${jsonPath.replace(
      /\.json$/,
      ".csv"
    )}`;
